    ...
    <div id="home" class="boxed-view">
        ...
        <section class="box">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<div class="text-dark-blue text-center fancy-heading">
							<h1 class="font-700">Previous  Reservations</h1>
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h1 class="text-center font-100 font-3x text-green"><span class="white">  <jst:out value="${search_name}" /></span></h1>
							<hr class="text-dark-blue size-30 center-me">
							<p> Previous reservations that checked in 
							<jst:if test="${not empty monthOfStay}">
								during the month of <jst:forEach items="${months_list}" var="month" varStatus="month_number">
									<jst:if test="${month_number.count eq monthOfStay }"><span class="text-orange font-10x">${month}</span></jst:if>
								</jst:forEach>
							</jst:if>
							<jst:if test="${not empty yearOfStay}">
							 during the year <span class="text-orange font-10x">${yearOfStay}</span>
							</jst:if>
							<jst:if test="${not empty firstName}">
							 with a first name like  <span class="text-orange font-10x">${firstName}</span>
							</jst:if>
							<jst:if test="${not empty lastName}">
							 with a last name like  <span class="text-orange font-10x">${lastName}</span>
							</jst:if>
							<jst:if test="${not empty roomTypeId}">
							 with a <jst:forEach items="${roomTypes}" var="type"><jst:if test="${roomTypeId eq type.roomTypeId}"><span class="text-orange font-10x">${type.roomTypeName}</span>  </jst:if> </jst:forEach>room
							</jst:if>
							</p>
							<br>			
						</div>
				...
				<jst:forEach items="${previous_reservations }" var="reservation" varStatus="counter">
